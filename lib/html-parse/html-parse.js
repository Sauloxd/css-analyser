...
function parse(userInput) {
  var basePath = userInput.basePath,
      glob = userInput.glob;
  var Preprocessors = (0, _preprocessors.preprocessorRetriever)(glob);
  return getAllTemplatesPaths(Preprocessors, basePath);
}

function getAllTemplatesPaths(Preprocessors, basePath) {
  var parsedClasses = {};
  return _bluebird.default.map(Preprocessors, function (Preprocessor) {
    return new Preprocessor.preprocessor({
      basePath: basePath,
      glob: Preprocessor.glob,
      logError: true
    }).compileAll({
